...
| Method | Endpoint | Quyền truy cập | Mô tả nghiệp vụ / Cấu trúc dữ liệu |
| :--- | :--- | :--- | :--- |
| **GET** | `/api/students/{studentId}/timetable` | Học sinh / Phụ huynh | Xem thời khóa biểu của học sinh trong tuần hoặc khoảng thời gian.<br>- **Query Parameters:** `startDate` (YYYY-MM-DD), `endDate` (YYYY-MM-DD). |
...
| **GET** | `/api/departments/{departmentId}/reports/grades` | Trưởng bộ môn | Tra cứu, xem báo cáo thống kê kết quả học tập (điểm thi, phân phối điểm) các môn học thuộc tổ chuyên môn. |
