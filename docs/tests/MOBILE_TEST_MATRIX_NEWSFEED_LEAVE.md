...
- [ ] **0.5** `flutter test` pass — ít nhất `relative_time_test`, `student_request_model_test`
...
- [ ] **0.7** Tạo ít nhất **1 tin khẩn** (`priority = Urgent`) để kiểm tra icon/highlight
...
| **Bước**    | Login **student01** → Trang chủ / Thông báo (feed Announcement) |
...
| ----------- | -------------------------------------------------------------- |
| **Tiền đề** | TC-TN01                                                        |
| **Bước**    | Login **student01** → Thông báo                                     |
...
### TC-SN01 · Màn Thông báo — feed từ API


|              |                                                                |
| ------------ | -------------------------------------------------------------- |
| **Màn hình** | Icon chuông / `notifications_view.dart` (1 list Announcement) |
| **Kỳ vọng**  | Danh sách từ `GET /api/announcement/my-feed`; không còn tab Hệ thống / NotificationLog |
...
| ----------------------------------------------------- | ---------------------------- |
| `test/core/utils/relative_time_test.dart`             | Parse UTC naive + relative VI |
