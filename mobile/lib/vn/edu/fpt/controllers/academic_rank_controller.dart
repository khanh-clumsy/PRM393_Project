...
class AcademicRankController extends GetxController with SubmitGuardMixin {
  ...
  Future<void> createRank(String name, double minScore, double maxScore) async {
    await runSubmitting(() async {
      try {
        ...
        Get.snackbar('Lỗi', ApiErrorHelper.messageFrom(e, fallback: 'Không thể thêm xếp loại'), backgroundColor: Colors.redAccent, colorText: Colors.white);
      }
    });
  }
  ...
  Future<void> updateRank(int id, String? name, double? minScore, double? maxScore) async {
    await runSubmitting(() async {
      try {
        ...
        Get.snackbar('Lỗi', ApiErrorHelper.messageFrom(e, fallback: 'Không thể cập nhật xếp loại'), backgroundColor: Colors.redAccent, colorText: Colors.white);
      }
    });
  }
